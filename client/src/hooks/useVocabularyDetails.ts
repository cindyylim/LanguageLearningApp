...
export const useVocabularyDetails = () => {
    ...
    const handleEditList = async (e: React.FormEvent) => {
        ...
        try {
            await axios.put(`${process.env.REACT_APP_API_URL}/vocabulary/${id}`, state.editListForm);
    ...
    const handleEditWord = async (e: React.FormEvent) => {
        ...
        dispatch({ type: 'ACTION_START' });
        try {
            await axios.put(`${process.env.REACT_APP_API_URL}/vocabulary/${id}/words/${state.editWordForm.id}`, state.editWordForm);
