...
describe('VocabularyService', () => {
    ...
    describe('generateAIList', () => {
        it('should generate AI vocabulary list', async () => {
            ...
            const insertedId = new ObjectId('507f1f77bcf86cd799439014');
            const mockWordIds = {
                0: new ObjectId('507f1f77bcf86cd799439015'),
                1: new ObjectId('507f1f77bcf86cd799439016'),
            };

            mockCollection.insertOne.mockResolvedValueOnce({ insertedId });
            mockCollection.insertMany.mockResolvedValue({ insertedIds: mockWordIds });
        ...
        it('should persist pinyin when provided by AI', async () => {
            const userId = 'user123';
            const listData = {
                name: 'Chinese Food',
                description: 'Generated by AI',
                targetLanguage: 'zh',
                nativeLanguage: 'en',
                prompt: 'common food words',
                wordCount: 2
            };

            const mockAIWords = [
                { word: '米饭', translation: 'rice', pinyin: 'mǐfàn', difficulty: 'easy' },
                { word: '面条', translation: 'noodles', pinyin: 'miàntiáo', difficulty: 'easy' }
            ];
            ...
            await VocabularyService.generateAIList(listData, userId);
    ...
    describe('updateWordProgress', () => {
        ...
        it('should mark word mastered immediately on manual Mastered click (not quiz path)', async () => {
            ...
            expect(mockCollection.insertOne.mock.calls[0][0].streak).toBe(6);
