...
router.put('/:id', validateObjectId(), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
...
router.post('/:id/words', validateObjectId(), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
...
router.post('/analyze-complexity', asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.post('/generate-ai-list', asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.post('/words/:wordId/progress', validateObjectId(), asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.get('/words/:wordId/progress', validateObjectId(), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { wordId } = req.params;
  const db = await connectToDatabase();
...
router.put('/:listId/words/:wordId', validateObjectId(), asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.delete('/:listId/words/:wordId', validateObjectId(), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { listId, wordId } = req.params;
  const db = await connectToDatabase();
  // Check list ownership
  const list = await db.collection('VocabularyList').findOne({ _id: new ObjectId(listId), userId: req.user!.id });
